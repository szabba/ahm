...
func TestFmtString(t *testing.T) {
	// given
	for name, tt := range fmtCases {
		t.Run(name, func(t *testing.T) {

			want := fmtWanted(t, name)

			// when
			out := ahm.FmtString(tt)

			// then
			diff := udiff.Unified("want", "got", want, out)
...
func TestFmt(t *testing.T) {
	...
	for name, tt := range fmtCases {
		t.Run(name, func(t *testing.T) {
			...
			diff := udiff.Unified("want", "got", want, buf.String())
...
func fmtWanted(t *testing.T, casename string) string {

	fname := fmt.Sprintf("testdata/fmt/%s.ahm", casename)
...
var fmtCases = map[string][]ahm.Node{
